...
// Add list rows that are description-based/non-serial in the provided list.
...
$sum = $m->query("SELECT COUNT(*) cnt, COALESCE(SUM(unit_cost*COALESCE(NULLIF(qty_physical_count,0),1)),0) total FROM rpcppe_batch_items WHERE id IN ($csv)")->fetch_assoc();

echo "Set rows: {$sum['cnt']}\n";
echo "Set total (current qty): " . number_format((float)$sum['total'],2) . "\n";

// Simulate correction for ID 19065 quantity from 2->1
$row19065 = $m->query("SELECT unit_cost, COALESCE(NULLIF(qty_physical_count,0),1) q FROM rpcppe_batch_items WHERE id=19065")->fetch_assoc();
$delta = (float)$row19065['unit_cost'] * ((int)$row19065['q'] - 1);
$sim = (float)$sum['total'] - $delta;

echo "ID 19065 current qty: {$row19065['q']} unit_cost: {$row19065['unit_cost']} delta if qty->1: " . number_format($delta,2) . "\n";
echo "Set total after qty fix: " . number_format($sim,2) . "\n";
echo "Expected target: 12,344,704.00\n";
echo "Difference: " . number_format($sim - 12344704.00,2) . "\n";
