...
$ids = [];
$stmt = $mysqli->prepare("SELECT id FROM rpcppe_batch_items WHERE batch_id = ? AND (serial_no LIKE ? OR item_description LIKE ?)");
foreach ($tokens as $t) {
    $like = '%' . $t . '%';
...
try {
    // Optional cleanup of this unique tag from batch 14 rows before retagging.
    $mysqli->query("UPDATE rpcppe_batch_items
                    SET remarks = TRIM(REPLACE(REPLACE(remarks, ' | $tag', ''), '$tag', ''))
                    WHERE batch_id = $batchId AND remarks LIKE '%$tag%'");
    ...
    $tagged = $mysqli->query("SELECT COUNT(*) cnt,
                              ...
                              WHERE batch_id = $batchId
                                AND remarks LIKE '%$tag%'")->fetch_assoc();

    $mysqli->commit();

    echo 'Unique-tag rows: ' . $tagged['cnt'] . PHP_EOL;
    echo 'Unique-tag total: ' . number_format((float)$tagged['total'], 2) . PHP_EOL;
