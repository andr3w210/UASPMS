...
DROP TEMPORARY TABLE IF EXISTS tmp_property_number_fix;

CREATE TEMPORARY TABLE tmp_property_number_fix AS
SELECT
    src.id,
    CONCAT(
        src.year_value, '-',
...
FROM (
    SELECT
        ...
        CASE
            WHEN TRIM(COALESCE(f.fund_code, '')) = '' THEN 'GEN'
            ELSE UPPER(TRIM(f.fund_code))
        END AS fund_segment,
        CASE
...
INNER JOIN tmp_property_number_fix t ON t.id = did.id
