...
if (!$db) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!csrf_verify()) {
            ...
            $recordId = (int) ($_POST['id'] ?? 0);
            $userId = current_user_id();
            $stmt = $db->prepare("UPDATE funds SET is_active = 1, updated_by = ?, updated_at = NOW() WHERE id = ?");
            if ($stmt) {
                $stmt->bind_param('ii', $userId, $recordId);
                $saved = $stmt->execute();
                $stmt->close();
                if ($saved) {
                    write_audit_log($db, [
                        ...
                        'module_name' => 'funds',
                        'record_type' => 'fund',
                        'action_name' => 'reactivate_fund',
                        'description' => 'Reactivated fund record.',
                        'new_values' => ['is_active' => 1],
                    ]);
                    set_flash('success', 'Fund reactivated successfully.');
                    redirect('modules/funds/index.php');
                }
            }
            $errors[] = 'Unable to reactivate the fund.';
...
<section class="master-data-page">
    <div class="card master-data-page-card">
        <div class="card-body p-4 p-xl-4">
                                ...
                                <div class="master-data-panel">
                                    ...
                                    <div class="master-data-panel-body">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-4"><label class="form-label">Fund Cluster</label><input type="text" class="form-control" name="fund_source" value="<?php echo h($form['fund_source']); ?>" placeholder="01, 05, 06"></div>
            ...
            <div class="master-data-table-shell"><div class="table-responsive mobile-table-frame master-data-table-scroll"><table class="table align-middle" id="dataTable"><thead><tr><th data-sort="code">Code <i class="bi bi-arrow-down-up text-muted small"></i></th><th data-sort="fund">Fund <i class="bi bi-arrow-down-up text-muted small"></i></th><th data-sort="source">Fund Cluster <i class="bi bi-arrow-down-up text-muted small"></i></th><th data-sort="status">Status <i class="bi bi-arrow-down-up text-muted small"></i></th><th data-sort="created">Created <i class="bi bi-arrow-down-up text-muted small"></i></th><th class="text-end">Actions</th></tr></thead><tbody><?php if ($funds): foreach ($funds as $fund): ?><tr data-status="<?php echo (int) $fund['is_active'] ? 'active' : 'inactive'; ?>"><td class="fw-semibold"><?php echo h($fund['fund_code']); ?></td><td><div class="fw-semibold"><?php echo h($fund['fund_name']); ?></div><small class="text-muted"><?php echo h($fund['description'] ?? ''); ?></small></td><td><?php echo h($fund['fund_source'] ?? ''); ?></td><td><span class="badge <?php echo (int) $fund['is_active'] === 1 ? 'text-bg-success' : 'text-bg-secondary'; ?>"><?php echo (int) $fund['is_active'] === 1 ? 'Active' : 'Inactive'; ?></span></td><td><div><?php echo h(date('M d, Y', strtotime($fund['created_at']))); ?></div><small class="text-muted"><?php echo h($fund['creator_name'] ?: 'System'); ?></small></td><td class="text-end"><div class="d-inline-flex flex-wrap justify-content-end gap-2"><a href="<?php echo base_url('modules/funds/index.php?edit=' . (int) $fund['id']); ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil-square"></i> Edit</a><?php if ((int) $fund['is_active'] === 1): ?><input type="hidden" name="_csrf" value="<?php echo h(csrf_token()); ?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?php echo (int) $fund['id']; ?>"><button type="submit" class="btn btn-sm btn-outline-warning"><i class="bi bi-slash-circle"></i> Deactivate</button></form><?php else: ?><input type="hidden" name="_csrf" value="<?php echo h(csrf_token()); ?>"><input type="hidden" name="action" value="reactivate"><input type="hidden" name="id" value="<?php echo (int) $fund['id']; ?>"><button type="submit" class="btn btn-sm btn-outline-success"><i class="bi bi-arrow-counterclockwise"></i> Reactivate</button></form><?php endif; ?><?php if (($_SESSION['user_role'] ?? '') === 'Administrator'): ?><input type="hidden" name="_csrf" value="<?php echo h(csrf_token()); ?>"><input type="hidden" name="action" value="hard_delete"><input type="hidden" name="id" value="<?php echo (int) $fund['id']; ?>"><button type="submit" class="btn btn-sm btn-outline-danger"><i class="bi bi-trash3"></i> Delete</button></form><?php endif; ?></div></td></tr><?php endforeach; else: ?><tr data-status="inactive"><td colspan="6" class="text-center text-muted py-4">No funds found yet.</td></tr><?php endif; ?></tbody></table></div>
