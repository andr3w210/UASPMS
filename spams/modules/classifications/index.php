...
if (!$db) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!csrf_verify()) {
            ...
            $duplicateStmt = $db->prepare("SELECT id, classification_code, classification_name, classification_group, account_code_id, is_active FROM classifications WHERE (classification_code = ? OR (account_code_id = ? AND LOWER(TRIM(classification_name)) = LOWER(TRIM(?)))) AND id != ? LIMIT 1");
            if ($duplicateStmt) {
                $recordId = (int) $form['id'];
                $accountCodeIdForDup = $form['account_code_id'] !== '' ? (int) $form['account_code_id'] : 0;
                $duplicateStmt->bind_param('sisi', $form['classification_code'], $accountCodeIdForDup, $form['classification_name'], $recordId);
                ...
                if ($duplicateRow) {
                    ...
                    $duplicateAccountCodeId = (int) ($duplicateRow['account_code_id'] ?? 0);
                    $duplicateAccountCode = '';
                    foreach ($accountCodes as $accountCode) {
                        if ((int) ($accountCode['id'] ?? 0) === $duplicateAccountCodeId) {
                            $duplicateAccountCode = trim((string) ($accountCode['account_code'] ?? ''));
                            break;
                        }
                    }
                    ...
                    if (strcasecmp($duplicateCode, $form['classification_code']) === 0) {
                        ...
                        $accountCodeLabel = $duplicateAccountCode !== '' ? $duplicateAccountCode : ('account #' . $duplicateAccountCodeId);
                        $errors[] = 'Inventory class name already exists under account code ' . $accountCodeLabel . ' as ' . $duplicateCode . ' (' . $duplicateGroup . ', ' . $duplicateStatus . ').';
                    }
                }
                $duplicateStmt->close();
            }
