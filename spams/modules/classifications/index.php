...
if (!$db) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!csrf_verify()) {
            ...
            if ($duplicateStmt) { $recordId = (int) $form['id']; $duplicateStmt->bind_param('ssi', $form['classification_code'], $form['classification_name'], $recordId); $duplicateStmt->execute(); if ($duplicateStmt->get_result()->fetch_assoc()) $errors[] = 'Inventory class code or name already exists.'; $duplicateStmt->close(); }
            if (!$errors) {
