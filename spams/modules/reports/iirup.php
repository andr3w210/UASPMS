...
if ($isPrint) {
    ...
    <div class="container-fluid py-3">
        ...
        <table class="table table-bordered align-middle"><thead><tr><th>Date Acquired</th><th>Particulars / Articles</th><th>Property No.</th><th class="text-end">Qty</th><th class="text-end">Unit Cost</th><th class="text-end">Total Cost</th><th class="text-end">Accumulated Depreciation</th><th class="text-end">Accumulated Impairment Losses</th><th class="text-end">Carrying Amount</th><th>Reason (COA)</th><th>Disposal Classification</th></tr></thead><tbody><?php if ($rows): foreach ($rows as $row): ?><tr><td><?php echo h(!empty($row['date_acquired']) ? date('M d, Y', strtotime((string) $row['date_acquired'])) : ''); ?></td><td><?php echo h(iirup_label($row)); ?></td><td><?php echo h($row['property_number'] ?? ''); ?></td><td class="text-end">1.00</td><td class="text-end"><?php echo h(number_format((float) ($row['unit_cost'] ?? 0), 2)); ?></td><td class="text-end"><?php echo h(number_format((float) ($row['unit_cost'] ?? 0), 2)); ?></td><td class="text-end"></td><td class="text-end"></td><td class="text-end"><?php echo h(number_format((float) ($row['unit_cost'] ?? 0), 2)); ?></td><td><?php echo h(disposal_reason_label($row['reason'] ?? '')); ?></td><td><?php echo h(disposal_reason_label($row['reason'] ?? '')); ?></td></tr><?php endforeach; else: ?><tr><td colspan="11" class="text-center text-muted py-4">No unserviceable property found.</td></tr><?php endif; ?></tbody><tfoot><tr><th colspan="8" class="text-end">Total Carrying Amount</th><th class="text-end"><?php echo h(number_format($totalValue, 2)); ?></th><th colspan="2"></th></tr></tfoot></table>
