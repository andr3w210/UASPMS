...
(function() {
    ...
    if(threadBody){ threadBody.addEventListener('click', function(e){ var btn = e.target.closest('.message-delete-btn'); if(!btn){ return; } var message = 'Remove this message from your view?'; var deleteMessage = function(){ var fd = new FormData(); fd.append('_csrf', <?php echo json_encode(csrf_token()); ?>); fd.append('message_id', btn.getAttribute('data-message-id') || '0'); fd.append('channel_message_id', btn.getAttribute('data-channel-message-id') || '0'); fetch(deleteUrl, { method:'POST', body:fd, credentials:'same-origin', headers:{'X-Requested-With':'XMLHttpRequest'} }).then(function(r){ return r.json(); }).then(function(data){ if(!data || data.ok !== true){ status(data && data.message ? data.message : 'Unable to remove the message.', true); return; } location.reload(); }); }; if(window.confirmAction){ e.preventDefault(); window.confirmAction({ title:'Confirm action', message: message, confirmText:'Confirm', onConfirm: deleteMessage }); return; } if(!confirm(message)){ return; } deleteMessage(); }); }
