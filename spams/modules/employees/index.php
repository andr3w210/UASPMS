...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function filterResponsibilityCodes() {
        ...
        Array.from(codeSelect.options).forEach(function (option, index) {
            ...
            if (matches && officeId !== '' && !preferredCodeId) preferredCodeId = option.value;
            if (!matches && option.selected) codeSelect.value = '';
        });
        var selectedOption = codeSelect.selectedOptions.length ? codeSelect.selectedOptions[0] : null;
        if (officeId !== '' && (!codeSelect.value || !selectedOption || selectedOption.hidden) && preferredCodeId !== '') codeSelect.value = preferredCodeId;
