...
<section class="section fuel-ris-workspace">
    <div class="card border-0 shadow-sm mb-3">
        <div class="card-body d-flex flex-wrap justify-content-between align-items-start gap-3">
            ...
            <div class="d-flex flex-wrap gap-2">
                <a href="<?php echo base_url('modules/trip_tickets/fuel_ris_create.php'); ?>" class="btn btn-success">
                    <i class="bi bi-plus-circle me-1"></i> Add Fuel RIS Entry
                </a>
    ...
    <!-- Keep this page list-first; entry/import actions are handled in fuel_ris_create.php. -->
    ...
    <div class="card border-0 shadow-sm mt-3">
        <div class="card-body">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
                <h5 class="card-title mb-0">Fuel RIS List</h5>
                <small class="text-muted">Latest 300 records for smoother page performance.</small>
            </div>
            <div class="table-responsive fuel-ris-table-wrap">
                <table class="table table-hover align-middle mb-0 fuel-ris-table">
                    <thead>
                        <tr>
                            <th>Date</th>
                            <th>RIS No.</th>
                    ...
                    <tbody>
                        <?php if (!$entries): ?>
                            <tr>
                                <td colspan="12" class="text-center text-muted py-5">No Fuel RIS entries found.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($entries as $entry): ?>
                                <tr>
                                    <td><?php echo h(format_date((string) $entry['ris_date'])); ?></td>
                                    <td class="fw-semibold"><?php echo h((string) $entry['ris_no']); ?></td>
                                    <td><?php echo h((string) ($entry['fuel_type'] ?? '')); ?></td>
                                    <td class="text-end fw-semibold"><?php echo number_format((float) ($entry['quantity'] ?? 0), 2); ?></td>
                                    <td><?php echo h((string) ($entry['unit'] ?? '')); ?></td>
                                    <td class="text-wrap"><?php echo h((string) ($entry['purpose'] ?? '')); ?></td>
                                    <td><?php echo h((string) ($entry['driver_name'] ?? '')); ?></td>
                                    <td>
                                        <div><?php echo h((string) ($entry['vehicle_plate_no'] ?? '')); ?></div>
                                        <small class="text-muted"><?php echo h((string) ($entry['vehicle_name'] ?? '')); ?></small>
                                    </td>
                                    <td><?php echo h((string) (($entry['gas_station_name'] ?? '') !== '' ? $entry['gas_station_name'] : ($entry['station_name'] ?? ''))); ?></td>
                                    <td class="text-end"><?php echo number_format((float) ($entry['amount'] ?? 0), 2); ?></td>
                                    <td class="text-center">
                                        <span class="badge <?php echo (string) ($entry['source_tag'] ?? '') === 'import' ? 'bg-info' : 'bg-secondary'; ?>">
                                            <?php echo h((string) ($entry['source_tag'] ?? 'manual')); ?>
                                        </span>
                                    </td>
                                    <td class="text-end">
                                        <form method="post" class="d-inline" onsubmit="return confirm('Delete this Fuel RIS entry?');">
