...
if (!$db) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'delete') {
            ...
            if ($departmentId > 0) {
                $userId = current_user_id();
                $stmt = $db->prepare("UPDATE departments SET is_active = 1, updated_by = ?, updated_at = NOW() WHERE id = ?");
                if ($stmt) {
                    $stmt->bind_param('ii', $userId, $departmentId);
                    $stmt->execute();
                    $stmt->close();
                    set_flash('success', 'Department reactivated successfully.');
                    redirect('modules/departments/index.php');
                }
            }

            $errors[] = 'Unable to reactivate the department.';
        }
    }
