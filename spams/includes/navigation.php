...
function spams_find_active_menu_item(string $path, array $menuGroups, ?string $role = null): ?array
{
    $resolvedRole = $role ?? (function_exists('current_user_role') ? current_user_role() : trim((string) ($_SESSION['user_role'] ?? ($_SESSION['role_name'] ?? ''))));

    foreach ($menuGroups as $group) {
        if (isset($group['roles']) && !in_array($resolvedRole, $group['roles'], true)) {
            continue;
        }
        ...
        foreach ($group['items'] as $item) {
            ...
            if (!nav_item_visible($item, $resolvedRole)) {
                continue;
            }

            if (nav_item_active($item['needle'], $path)) {
                return [
...
$menuGroups = [
    ...
    [
        ...
        'items' => [
            ...
            ['label' => 'PAR/ICS Reconciliation', 'path' => 'modules/property/par_ics_reconciliation.php', 'needle' => '/property/par_ics_reconciliation', 'icon' => 'bi bi-file-earmark-check', 'roles' => ['Administrator', 'Property Officer']],
